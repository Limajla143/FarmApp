...
namespace MyAPI.Extensions
{
    public static class UserManagerExtension
    {
        ...
        public static async Task<AppUser> FindUserByClaimsRefreshTokenAddress(this UserManager<AppUser> userManager, ClaimsPrincipal user)
        {
            var email = user.FindFirstValue(ClaimTypes.Email);

            return await userManager.Users.Include(x => x.Address).Include(r => r.RefreshTokens)
                .SingleOrDefaultAsync(x => x.Email == email);
        }
    }
}
